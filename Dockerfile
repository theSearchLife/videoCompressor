...
RUN apk add --no-cache bash ffmpeg mediainfo zsh
COPY . .
...
RUN curl --fail --location --retry 5 --retry-all-errors --retry-delay 2 \
    ...
    rm -rf /tmp/vmaf-${VMAF_VERSION} /tmp/vmaf.tar.gz

RUN ARCH=$(dpkg-architecture -qDEB_HOST_MULTIARCH) && \
    curl --fail --location --retry 5 --retry-all-errors --retry-delay 2 \
        --output /tmp/ffmpeg.tar.bz2 \
    ...
    make -j$(nproc) && \
    make install && \
    rm -rf /tmp/ffmpeg-${FFMPEG_VERSION} /tmp/ffmpeg.tar.bz2
